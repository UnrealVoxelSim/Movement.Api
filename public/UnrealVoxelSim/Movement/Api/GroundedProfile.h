...
namespace UnrealVoxelSim::Movement::Api
{
	struct GroundedProfile final
	{
		...
		Math::Api::FixedPointScalar SwimmingSpeed{};
		...
		[[nodiscard]] constexpr bool IsValid() const noexcept
		{
			return Id.IsValid() && Width != 0 && Length != 0 && Height != 0 && MaximumSpeed.Raw() > 0 &&
				JumpSpeed.Raw() > 0 && Gravity.Raw() < 0 && SwimmingSpeed.Raw() >= 0 && CollisionSkin.Raw() >= 0 &&
		...
		[[nodiscard]] constexpr bool CanSwim() const noexcept { return SwimmingSpeed.Raw() > 0; }
	};
}
